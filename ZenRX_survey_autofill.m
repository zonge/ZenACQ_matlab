    ...
    if strcmp(Cmp{1,1}(1:2),'TX')
        ...
        if isempty(tx_exist) % IF IT DOESN"T EXIST BEEP AND MESSAGE
           ...
           choice = listdlg('PromptString','Available TX:',...
                ...
                'ListString',tx_list);
            if isempty(choice)
                eventdata.Source.Data{eventdata.Indices(1),eventdata.Indices(2)}='';
            else
                ...
                eventdata.Source.Data{eventdata.Indices(1),eventdata.Indices(2)}=tx_list{choice};
            end
...
if strcmp(eventdata.NewData(1:2),'Hz')
    ...
    for i=1:size(eventdata.Source.Data,1)
       ...
       if strcmp(eventdata.Source.Data{i,1}(1),'T')
            handles.TX.type{i}=eventdata.Source.Data{i,6};
            handles.TX.sn{i}=eventdata.Source.Data{i,7};
       end
    end
